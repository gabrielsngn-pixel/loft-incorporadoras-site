...
const C = {
  orange: '#FF774A',      
  orangeL: '#FF9570',
  orangeDim: 'rgba(255,119,74,0.08)',
  orangeGlow: 'rgba(255,119,74,0.22)',
  orangeBorder: 'rgba(255,119,74,0.22)',
  bg: '#060606',
  s1: '#0d0d0d',
  s2: '#111111',
  s3: '#181818',
...
const LoftMark = ({ height = 28, color = '#FF774A' }) => (
  ...
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M29.5614 49.2055H34.3403V25.4497H41.1097L42.4645 20.692H34.3403V14.1486C34.3403 11.176 36.7694 8.75766 39.7553 8.75766V4C34.1344 4 29.5614 8.55308 29.5614 14.1486V20.692H27.1512C26.082 16.4599 22.2273 13.3176 17.6491 13.3176C12.2471 13.3176 7.85241 17.6927 7.85241 23.0708C7.85241 28.4489 12.2471 32.8241 17.6491 32.8241C22.2273 32.8241 26.082 29.6817 27.1512 25.4497H29.5614V49.2055ZM49.6548 20.692H53V25.4497H49.6548V41.1138H44.8759V14.4709H49.6548V20.692ZM17.6491 17.5994C14.6189 17.5994 12.1534 20.054 12.1534 23.0708C12.1534 26.0877 14.6189 28.5421 17.6491 28.5421C20.6795 28.5421 23.145 26.0877 23.145 23.0708C23.145 20.054 20.6795 17.5994 17.6491 17.5994ZM0 27.6272C0 34.8883 5.9335 40.7954 13.2266 40.7954V36.0377C8.56858 36.0377 4.7789 32.2644 4.7789 27.6272V14.4707H0V27.6272Z"
      fill={color}
    />
  </svg>
);

const LoftLogo = ({ markHeight = 28, showTagline = true }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
    <LoftMark height={markHeight} color="#FF774A" />
    {showTagline && (
      <>
        <div style={{ width: 1, height: markHeight * 0.9, background: '#2e2e2e', flexShrink: 0 }} />
        <span style={{
          fontSize: 9,
          color: '#5a5a5a',
          fontWeight: 700,
...
const ContactModal = ({ isOpen, onClose }) => {
  ...
  const inputStyle = (id) => ({
    width: '100%',
    background: C.s2,
    border: `1px solid ${focus === id ? C.orange : C.borderB}`,
    color: '#fff',
    padding: '14px 16px',
    borderRadius: 10,
    fontSize: 14,
    ...
    boxShadow: focus === id ? `0 0 0 3px ${C.orangeDim}` : 'none',
      ...
      {isOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          ...
          <motion.div
            ...
            style={{
              position: 'relative', width: '100%', maxWidth: 460, background: C.s1,
              border: `1px solid ${C.border}`, borderRadius: 24, padding: '40px 32px',
              boxShadow: `0 48px 120px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05)`,
              overflow: 'hidden'
            }}
          >
            <Glow style={{ width: 400, height: 400, top: -200, left: -100, opacity: 0.15 }} />
            
            <button onClick={onClose} style={{ 
              position: 'absolute', top: 24, right: 24, background: C.s2, border: `1px solid ${C.borderB}`, 
              color: C.t2, cursor: 'pointer', width: 32, height: 32, borderRadius: '50%', 
              display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' 
            }}
              onMouseEnter={e => { e.currentTarget.style.color = '#fff'; e.currentTarget.style.borderColor = C.t3; }}
              onMouseLeave={e => { e.currentTarget.style.color = C.t2; e.currentTarget.style.borderColor = C.borderB; }}>
              <X size={16} />
            </button>

            <div style={{ position: 'relative', zIndex: 10 }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: C.orangeDim, border: `1px solid ${C.orangeBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                <MessageCircle size={24} color={C.orange} />
              </div>
              <h3 style={{ fontSize: 32, fontWeight: 900, color: '#fff', margin: 0, letterSpacing: '-1px', ...SF }}>
                Fale com especialista
              </h3>
              <p style={{ fontSize: 14, color: C.t2, marginTop: 8, marginBottom: 32, lineHeight: 1.6 }}>
                Conecte-se diretamente com o gestor do canal para estruturarmos o melhor modelo para o seu volume de repasse.
              </p>

              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: C.t2, marginBottom: 8 }}>Nome completo</label>
                  <input required type="text" placeholder="Ex: João Silva" 
                    value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})}
                    onFocus={() => setFocus('nome')} onBlur={() => setFocus(null)}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: C.t2, marginBottom: 8 }}>Mensagem</label>
                  <textarea required placeholder="Como podemos acelerar sua operação?" rows={3}
                    value={formData.mensagem} onChange={e => setFormData({...formData, mensagem: e.target.value})}
                    onFocus={() => setFocus('mensagem')} onBlur={() => setFocus(null)}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          ...
          <motion.div
            ...
            <motion.div variants={fadeUp} style={{ display: 'flex', gap: 12 }}>
              <motion.button onClick={openModal} whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.98 }}
                style={{
      ...
      <div style={{ paddingLeft: 24 }}>
        ...
        <div style={{ height: 5, background: C.s3, borderRadius: 99, overflow: 'hidden' }}>
          <motion.div initial={{ width: 0 }} animate={{ width: started ? `${afterPct}%` : 0 }}
            ...
            style={{
              height: '100%', borderRadius: 99, background: improved ? C.green : C.orange,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
          ...
          <div style={{ width: '100%', maxWidth: 560, margin: '0 auto', aspectRatio: '1' }}>
            <svg viewBox={`0 0 ${VB} ${VB}`} width="100%" height="100%" style={{ display: 'block', overflow: 'visible' }}>
              ...
              <motion.circle cx={CX} cy={CY} r={R} fill="none" stroke={C.orange} strokeWidth="1.8" strokeOpacity="0.6" strokeLinecap="round"
              ...
              {modules.map((mod, i) => {
                ...
                return (
                  <line key={`spoke-${i}`} x1={CX} y1={CY} x2={nx} y2={ny} stroke={isActive ? C.orange : 'rgba(255,90,0,0.16)'}
                    strokeWidth={isActive ? 1.5 : 1} strokeDasharray="4 8" style={{ transition: 'stroke 0.3s, stroke-width 0.3s' }} />
                );
              })}
              ...
              {modules.map((mod, i) => {
                ...
                const ny = CY + Math.sin(rad) * R;
                return (
                  <motion.circle key={`dot-${i}`} r="3.5" fill={C.orange} filter="url(#dotGlow)"
                    animate={{ cx: [CX, nx], cy: [CY, ny], opacity: [0.85, 1, 0] }}
                    transition={{ duration: 2.2, repeat: Infinity, delay: i * 0.37, ease: 'easeInOut' }} />
                );
              })}

              <circle cx={CX} cy={CY} r={82} fill={C.orange} opacity="0.18" filter="url(#centerGlow)" />
              <circle cx={CX} cy={CY} r={70} fill={C.orange} />
              <ellipse cx={CX - 14} cy={CY - 20} rx={38} ry={28} fill="rgba(255,255,255,0.07)" />
              <circle cx={CX} cy={CY} r={82}  fill="none" stroke={C.orange} strokeWidth="13" strokeOpacity="0.10" />
              <circle cx={CX} cy={CY} r={98}  fill="none" stroke={C.orange} strokeWidth="6"  strokeOpacity="0.05" />

              <g transform={`translate(${CX - 23}, ${CY - 27.5})`}>
                <svg xmlns="http://www.w3.org/2000/svg" width="46" height="47" viewBox="0 0 53 54" fill="none">
                  ...
                  <React.Fragment key={mod.id}>
                    <foreignObject x={nx - NODE / 2} y={ny - NODE / 2} width={NODE} height={NODE} style={{ overflow: 'visible' }}>
                      <motion.div xmlns="http://www.w3.org/1999/xhtml"
                        ...
                        whileHover={{ scale: 1.13 }} whileTap={{ scale: 0.9 }}
                        onClick={() => setActive(isActive ? null : mod.id)}
                        style={{
                          width: NODE, height: NODE, borderRadius: 15, cursor: 'pointer',
                          background: isActive ? C.orange : '#0f0f0f',
                          border: `1px solid ${isActive ? C.orange : '#272727'}`,
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          boxShadow: isActive ? `0 0 32px rgba(255,90,0,0.5), 0 0 0 7px rgba(255,90,0,0.1)` : '0 6px 28px rgba(0,0,0,0.6)',
                    ...
                    <foreignObject x={lx - LABEL_W / 2} y={ly - LABEL_H / 2} width={LABEL_W} height={LABEL_H} style={{ overflow: 'visible', pointerEvents: 'none' }}>
                      <div xmlns="http://www.w3.org/1999/xhtml" style={{ textAlign: 'center' }}>
                        {lines.map((ln, li) => (
                          <div key={li} style={{
                            fontSize: 11, fontWeight: 700, lineHeight: 1.35, color: isActive ? '#fff' : '#6e6e6e',
                            whiteSpace: 'nowrap', fontFamily: "-apple-system, 'Helvetica Neue', sans-serif", transition: 'color 0.25s',
                          }}>{ln}</div>
                        ))}
                      </div>
                    </foreignObject>
  ...
  return (
    <section style={{ padding: '140px 40px', background: C.s1, borderTop: `1px solid ${C.border}`, position: 'relative', overflow: 'hidden' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
        ...
        <motion.button onClick={openModal} onHoverStart={() => setHover(true)} onHoverEnd={() => setHover(false)}
